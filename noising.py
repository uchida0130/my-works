...
def convolution(impfile, voicefile):
    Fs = 16000
    norm = 32768.
    fs,imp = read(impfile)
    imp = imp/norm
    ...
    return Y, 10 * np.log10(H_[maxF]*np.conj(H_[maxF]))
...
def samplefileread(snr,noise_num):
    ...
    for i, voice in enumerate(voicefile[noise_num*1000:(noise_num+1)*1000]):
        ...
        for ch_iter in xrange(ch_number):
            ...
            Y = noise_add(noise, noise_list[noise_num], noise_num*1000+i, ch_iter, H_, voice,snr,start)
...
def getteachdata():
    ...
    for i, voice in enumerate(voicefile[0:10000]):
        ...
        # print "proc: ch{}".format(ch_iter+1)
        impfile   = glob.glob("ch1/imp_data/*.wav")
        for j, imp in enumerate(impfile):
            Y,_ = convolution(imp,voice)
            write("teacher/No{0:04d}.wav".format(i),Fs,Y)
...
def testdata_gen(snr,noise_num):
    ...
    for ch_iter in xrange(ch_number):
        ...
        Y = noise_add(noise, noise_list[noise_num], noise_num*1000+i, ch_iter, H_, voice,snr,start)
        write("ch{0:d}/test_data/{1:d}dB_noise{2:02d}_test.wav".format(ch_iter+1,snr,noise_list[noise_num]),Fs,Y)
    f.close
